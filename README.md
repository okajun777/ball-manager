...
- ダッシュボード（今日・今週・推奨・調子の推移・直近共有）
- マイボール（購入日・ショップ・ドリラー・表面メンテ履歴・引退・カタログ画像・成績）
- カタログ（メーカー・カバー・コアで絞り込み、所持追加）
- スコア入力（合計点 / フレーム、編集、練習/大会、前回コピー、レーン、共有、ゲーム複製、同日警告）
- 分析（期間フィルタ、練習/大会比較、ボール別練習vs大会、最高ゲーム、月別、X率、店舗・オイル・レーン別、大会まとめ、推移）
- 攻略AI（オイル条件 ＋ 実績、画像/PDF読取、AI解説、スコア入力へ連携）
- ROUND1プロショップ商品ビューア連携
- 表面メンテリマインダー（ダッシュボード / 任意で通知）
- JSONバックアップ / スコアCSV書き出し / 招待コード参加
- スコア入力の店舗・オイル・大会名・レーン履歴クイック選択
- スコア入力の店舗・オイル既定値
- 分析の自動「気づき」 / レーン別平均
